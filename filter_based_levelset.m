...
function filter_based_levelset(nelx, nely,V_max, penal,rmin) %% PARAMETERSS
    ...
    T = 6;         % The time at which volume fraction is expected to reach V_max 
    ...
    KME0=E0/(1-nu^2)^2/24* ...
         ((3*nu^2-2*nu+3)*[A11 A12;A12' A11]+(-nu^2+6*nu-1)*[B11 B12;B12' B11]);
    ...
    for It=1: Tinf/dt
        ...
        % Level set function for each node in matrix form 
        phi_n_m = reshape(phi_n_v, nely+1, nelx+1); 
        % Filtering of the level set function. Refer to "Matlab Online Help,
        % [online] Available: http: //www.mathworks. com Initial" for more details on the function imfilter.
        phi_n_m = imfilter(phi_n_m, H,'same');
        % Mapping the level set function onto elements in matrix form (Otomori 2015).
        phi_e_m = 0.25*(phi_n_m(1: end-1,1: end-1)+phi_n_m(2:end,1:end-1) + ... 
                        phi_n_m(1:end-1,2:end) + phi_n_m(2:end, 2:end) ) ;
        % Updating material distribution. Refer to "Matlab Online Help,
        % [online] Available:http://www. mathworks.comInitial"formore details on the function sigmf.
        str(:,:) = sigmf(phi_e_m(:,:), [penal+It*0 0]); 
        V = sum(str(: ))/(nely*nelx);
        ...
        J1(It) = full(F(2*((nely+1)*(nelx+1)-nely/2),1))*U(2*((nely+1)*(nelx+1)-nely/2),1);  
        alfa = 1/J1 (It) * (nely+1) * (nelx+1) ;
        % Calculating the topological derivative on each element.
        TDJ_e = (ep_E+str*(1- ep_E)).*reshape(sum((U(edofMat)*KME0).*U(edofMat), 2), nely, nelx); 
        ...
        % Mapping the topological derivative onto nodes in matrix form (Otomori 2015).
        TDJ_n = 0.25*(td2(1:end-1,1:end-1) +td2(2:end, 1:end-1) +td2(1:end-1,2:end) +td2(2:end,2:end));
        % Updating the level set function in the vector form.
        phi_n_v = phi_n_v+(TDJ_n(:) *alfa-Lambda) *dt; 
